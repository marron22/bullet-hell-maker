...
- プレイヤーは通常移動に加えて Shift キーで短時間の高速ダッシュができます。譜面はこのダッシュを前提にできますが、連続して常にダッシュしないと避けられない配置は避けます。
...
- `color` は `#rrggbb` 形式です。指定しない場合はパッケージの既定色 `#f82469` になります。
...
- プレイヤーは通常移動に加えて Shift キーで短時間の高速ダッシュができる
